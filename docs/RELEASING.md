...
Use four-part numeric MSIX versions such as `0.9.0.0`. Create a matching Git tag such as `v0.9.0`. The release workflow converts a three-part tag to its four-part MSIX version.

v0.9.0 pull-request artifacts use the reserved package lane `0.8.1.<run>` while reporting product version `0.9.0`. They install over public v0.8.1 for Windows verification, while the final `0.9.0.0` package remains numerically newer than every test build. For each later release, reserve the latest public package lane; never give a test package a version that would make the final package look like a downgrade.
...
```powershell
.\scripts\Build-Release.ps1 -Version 0.9.0.0
```
