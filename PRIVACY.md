...
The installer contains the original CRNY track **Relay Online**. Setup extracts it only to its automatically cleaned temporary directory and prepares a private Windows Media Player instance fixed at 10% volume, with an independently volume-limited Windows MCI fallback. Fresh installs start playback; verified updater mode starts muted until the user selects **Play music**. Neither path changes the user's Windows master volume, and playback is not started if a safe per-player volume cannot be enforced. The track is not sent over the network or retained as a separately installed file. The direct SoundCloud track page opens in the default browser only if the user selects that button.
...
The update request uses the app version and normal network metadata such as the user's IP address; it contains no OpenXBL key, Discord webhook, Xbox/Steam account identifier, achievement history, or settings. Update checks occur at startup and approximately every six hours. A verified release download can start automatically under the documented launch/required-update policy or after the user selects **Update now**.
