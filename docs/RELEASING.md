...
Use four-part numeric MSIX versions such as `0.10.0.0`. Create a matching Git tag such as `v0.10.0`. The release workflow converts a three-part tag to its four-part MSIX version.

v0.10.0 pull-request artifacts use the reserved package lane `0.9.0.<run>` while reporting product version `0.10.0`. They install over public v0.9.0 for Windows verification, while the final `0.10.0.0` package remains numerically newer than every test build. For each later release, reserve the latest public package lane; never give a test package a version that would make the final package look like a downgrade.
...
```powershell
.\scripts\Build-Release.ps1 -Version 0.10.0.0
```
...
4. Verify installer connect-now/skip and Steam-only paths, encrypted handoff deletion, desktop-shortcut choice, first baselines, Discord test, and real Xbox account sync. Run two consecutive Xbox syncs and confirm that no pre-baseline or newly revealed historical achievement reaches Discord or the Signal Strip. Post an Xbox unlock from device A, start device B later, and confirm B silently reconciles it; also confirm one genuinely live modern unlock and one untimestamped Xbox 360 unlock post exactly once and display exactly one local Signal Strip, and that a forced failed live delivery remains retryable across an app restart. Confirm a sleep/gap longer than ten minutes begins a silent epoch. On Xbox PC Game Pass, verify an achievement carrying recognized singular Windows evidence appears as **Xbox PC** in the Discord field, Collector Card and Signal Strip; confirm plural/title availability metadata alone remains **Xbox**, while genuine direct console and Xbox 360 controls retain their labels. For Steam, baseline a history-heavy game, observe one real live unlock, verify a restart/offline unlock stays silent, simulate a failed webhook and verify its pending live transition retries once, and exercise the x64 helper/package check on every supported architecture available. Inspect artwork-backed, icon-only and fallback Collector Cards unexpanded at normal Discord desktop and mobile size: foreground artwork must be recognisable, small/square icons must remain contained, achievement/game/description text must be readable and the rarity percentage must remain dominant. Queue consecutive sample/live events and confirm the overlay remains click-through, non-activating, silent, top-centred, bounded to the active display and sequential; disable the overlay in Settings and verify Discord delivery continues. Finish with startup, running-app update discovery/download, muted updater launch with opt-in 10% Play/Pause, updater cancellation, `/UPDATE=1` state/shortcut preservation, required-policy monitoring suspension, and uninstall checks.
