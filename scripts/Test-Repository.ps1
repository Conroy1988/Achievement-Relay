...
$manifestText = $manifestText.Replace('__VERSION__', '0.9.0.0').Replace('__ARCHITECTURE__', 'x64')
...
if (-not $releaseWorkflowText.Contains("'.exe'") -or
    ...
    -not $releaseWorkflowText.Contains('default: v0.9.0') -or
...
$releaseNotesText = Get-Content -LiteralPath (Join-Path $repositoryRoot 'docs\RELEASE-NOTES-0.9.0.md') -Raw
...
if ($officialUpdatePolicy.schemaVersion -ne 1 -or
    ...
    -not $appProjectText.Contains('<Version>0.9.0</Version>') -or
    -not $appProjectText.Contains('<FileVersion>0.9.0.0</FileVersion>') -or
    -not $appProjectText.Contains('<AssemblyVersion>0.9.0.0</AssemblyVersion>') -or
    -not $bridgeProjectText.Contains('<Version>0.9.0</Version>') -or
    -not $bridgeProjectText.Contains('<FileVersion>0.9.0.0</FileVersion>') -or
    -not $bridgeProjectText.Contains('<AssemblyVersion>0.9.0.0</AssemblyVersion>') -or
    -not $installerText.Contains('#define AppVersion "0.9.0"') -or
    -not $buildReleaseText.Contains("[string] `$Version = '0.9.0.0'") -or
    -not $buildMsixText.Contains("[string] `$Version = '0.9.0.0'") -or
    -not $buildInstallerText.Contains("[string] `$Version = '0.9.0'") -or
    -not $buildInstallerText.Contains("[string] `$MsixVersion = '0.9.0.0'") -or
    -not $mainWindowXaml.Contains('Text="Version 0.9.0"') -or
    -not $mainWindowText.Contains('?? "0.9.0"') -or
    -not $discordClientVersionText.Contains('ProductInfoHeaderValue("AchievementRelay", "0.9.0")') -or
    -not $openXblClientVersionText.Contains('ProductInfoHeaderValue("AchievementRelay", "0.9.0")') -or
    -not $steamRarityClientVersionText.Contains('ProductInfoHeaderValue("AchievementRelay", "0.9.0")') -or
    -not $releaseNotesText.Contains('# Achievement Relay v0.9.0') -or
    ...
    -not $releaseNotesText.Contains('AchievementRelay_0.9.0.0_x64.msix') -or
    -not $changelogText.Contains('## [0.9.0] - 2026-09-12') -or
    -not $changelogText.Contains('Collector Card showcase') -or
    -not $changelogText.Contains('Xbox PC Game Pass')) {
    throw 'The v0.9.0 application, showcase/platform release notes and Steam bridge must retain the official v0.4.0 update baseline.'
}
...
if (-not $ciWorkflowText.Contains('0.8.1.${{ github.run_number }}') -or
    -not $ciWorkflowText.Contains('APPLICATION_VERSION: "0.9.0"') -or
    -not $ciWorkflowText.Contains('AchievementRelay-v0.9.0-r${{ github.run_number }}-windows-test') -or
