...
Every app start creates a new in-memory Xbox live-delivery epoch. A normal successful poll keeps that epoch alive; a gap longer than the configured continuity allowance (at least ten minutes), or a backwards system-clock jump, resets it. Identity differences whose usable provider time is at or before the epoch are retained silently. An untimestamped Xbox 360 identity is eligible only when its summary change was first observed after a successful poll in that uninterrupted session. The queue durably stores that live evidence and its original epoch so a provider/Discord failure remains retryable across an app or updater restart. Work migrated from an older schema has no such proof and fails closed.

After a verified identity baseline exists, candidate events are the set difference between the current complete identity set and the saved set. Xbox 360 responses with a missing or `0001-01-01` time remain valid only under the direct live-evidence rule above and use the observation time in Discord with an explicit estimated-time footer. Before an identity baseline exists, counts and Gamerscore can never infer a post: only a usable timestamp strictly after the current live-delivery epoch may be delivered, while all old or unproven identities are stored silently. `EventLedger` prevents already handled deterministic identities from posting twice on that Windows account. The cursor/title snapshot is not advanced past a pending provider or Discord delivery.
...
Xbox device metadata is also treated conservatively. Legacy response shape and the dedicated legacy route are authoritative for **Xbox 360**. Otherwise singular `earnedPlatform`, `deviceType` and `device` are checked together before singular `platform`; recognized event-level aliases must agree, after which a Windows token produces **Xbox PC** and a console token produces **Xbox Console**. An unknown or conflicting alias stops at generic **Xbox**. Plural achievement/title `platforms` and `devices` arrays are availability metadata and can never manufacture either specific label, while all other absent or weak evidence becomes **Xbox** at delivery. Steam events use **Steam** directly. Platform evidence is display metadata and never becomes part of an event ID, and no local-process or device-presence inference is used for Xbox classification.

The renderer creates one bounded 1200×675 PNG in memory. It gives the foreground achievement image a 400×250 showcase bay and can use suitable wide hero or achievement artwork as the ambient layer. Small or square icons remain contained and are never promoted into a stretched full-card background. The ambient image keeps a lighter global treatment while local dark panels protect the details and rarity text. Achievement titles fit within 46–68 px, descriptions use 32 px type and the rarity percentage can reach 96 px. If no suitable ambient artwork exists, the renderer draws the complete Achievement Relay fallback. The multipart attachment includes a concise author-controlled description, and the achievement name, game, platform, percentage and tier also remain in the normal Discord embed because client support for attachment descriptions can vary.
